...
def compare3(final_result, final_candy):
    if final_result == -2:
        final_candy = money - 2 * bet1
        return int(final_candy)
    elif final_result == +2:
        final_candy = money + 2 * bet1
        return int(final_candy)
    elif final_result == -1:
        final_candy = money - bet1
        return int(final_candy)
    elif final_result == 1:
        final_candy = money + bet1
        return int(final_candy)
...
def bet_result(bet, final_of_final_result, last_final_candy):
    if final_of_final_result == -2:
        return print(f'\nBạn thua đậm mất {2 * bet} viên kẹo và còn {last_final_candy} viên trong túi. 😱')
    elif final_of_final_result == +2:
        return print(f'\nBạn thắng đậm thêm {2 * bet} viên kẹo và có {last_final_candy} viên trong túi. 😳')
    elif final_of_final_result == -1:
        return print(f'\nBạn thua mất {bet} viên kẹo và còn {last_final_candy} viên trong túi. 🤏')
    elif final_of_final_result == 1:
        return print(f'\nBạn thắng thêm {bet} viên kẹo và có {last_final_candy} viên trong túi. 😮')
    else:
...
while reset_game:
    ...
    while again_again:
        ...
        alive = True
        while alive:
            bet1 = int(input('Số kẹo bạn muốn đánh đổi: '))
            if bet1 > 1000:
                print('Bạn giỡn mặt hả, gì mà cược bằng cả tính mạng thế? Cho cơ hội làm lại đó 😑')
                time.sleep(0.5)
            elif bet1 > money:
                print(f'Máu đó! Bạn đã liều mình cược {bet1} viên kẹo! Dù không đủ 😃')
                alive = False
            elif bet1 < money:
                print(f'Bạn đã an toàn cược {bet1} viên kẹo, nước đi an toàn đấy. 😊')
                alive = False
        time.sleep(0.5)
        print('\nBắt đầu nè.')
        time.sleep(0.5)
        ...
        bet_result(bet=bet1, final_of_final_result=last_final_result, last_final_candy=final_of_final_candy)
        time.sleep(0.5)
